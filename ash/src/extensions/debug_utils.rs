...
impl DebugUtils {
    ...
    pub unsafe fn debug_utils_set_object_name_ext<H: vk::Handle>(
        &self,
        device: vk::Device,
        object: H,
        name: &CStr,
    ) -> VkResult<()> {
        let err_code = self.debug_utils_fn.set_debug_utils_object_name_ext(
            device,
    ...
    pub unsafe fn debug_utils_set_object_tag_ext<H: vk::Handle>(
        &self,
        device: vk::Device,
        object: H,
        tag_name: u64,
        ...
        let err_code = self.debug_utils_fn.set_debug_utils_object_tag_ext(
            device,
            &vk::DebugUtilsObjectTagInfoEXT {
                object_type: H::TYPE,
                object_handle: object.as_raw(),
                tag_name,
                tag_size: tag.len(),
                p_tag: tag.as_ptr() as _,
                ..Default::default()
            },
        );
        match err_code {
